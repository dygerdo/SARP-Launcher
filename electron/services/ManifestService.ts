...
import type { IManifestService, RemoteManifest } from "../../src/types/mods"
...
export class ManifestService implements IManifestService {
  ...
  private readonly DEFAULT_MANIFEST_URL =
    (process.env.VITE_CDN_URL || "https://sarp-public.b-cdn.net") + "/manifest.json"
  ...
  public async fetchManifest(force: boolean = false): Promise<RemoteManifest> {
    ...
    try {
      ...
      console.warn(
        "[ManifestService] Fetch failed, falling back to cache if available.",
        error.message,
      )
  ...
  private validateManifest(manifest: any): void {
    ...
    if (!manifest.version || !Array.isArray(manifest.mods))
      throw new Error("Manifest missing critical fields.")
